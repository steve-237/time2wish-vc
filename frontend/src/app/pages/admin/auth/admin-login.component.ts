    ...
    <div class="admin-auth-container">
      <div class="admin-auth-card">
        <div class="admin-auth-header">
          ...
          <h1>Time2Wish</h1>
          <p class="subtitle">{{ t9n.t('admin.login.title') }}</p>
        </div>

        <form (ngSubmit)="onSubmit()" #loginForm="ngForm" class="admin-form">
          <div class="form-group">
            <label for="email">{{ t9n.t('admin.login.email') }}</label>
          ...
          <div class="form-group">
            <label for="password">{{ t9n.t('admin.login.password') }}</label>
            <div class="input-with-icon">
          ...
          <button type="submit" class="admin-btn-primary" [disabled]="!loginForm.form.valid || isLoading()">
            @if (isLoading()) {
              ...
              <span class="material-symbols-outlined">login</span> {{ t9n.t('admin.login.btn') }}
            }
          </button>
        </form>

        <div class="admin-auth-footer">
          <a routerLink="/admin/register" class="admin-link">{{ t9n.t('admin.login.link_register') }}</a>
